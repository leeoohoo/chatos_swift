...
`VisualSessionActor` 负责读取、校验、归属映射、去重和过期；`@MainActor VisualSessionModel` 只负责可见状态。收到新的 `session.id` 时自动切换为 expanded，同一 session 的新 `frameSequence` 只更新帧，不打断用户的收起选择。
